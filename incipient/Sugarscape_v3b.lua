...
Sugarscape = Model{
    ...
    init = function(model)
        ...
        model.timer = Timer{
            ...
            Event{start = 0, period = 50, priority = "low", action = function(event)
                -- Saving maps periodically
                local time = event:getTime()
                model.map3:save("sugar_howfit_"..time.."_ch3.bmp")
                results = {}
                i=1
                forEachAgent(model.society,function(self)
                    line = {age    = self.age,
                            vision = self.vision,
                            metab  = self.metabolism,
                            sugar  = self.sugar
                            }
                    results[i] = line
                    i = i + 1
                end)
                CSVwrite(results, "Sugar_Ch3_"..time..".csv", ";")

            end},
            Event{start = model.finalTime, priority = "low", action = function(event)
