local function hasHuman(agents)
	local result = false
	forEachElement(agents, function(idx, agt, _)
...
local function cellsOccupationPercetage(cs)
	local emptys = 0
	local houses = 0
	local publics = 0
	...
	forEachCell(cs, function(cell)
		if cell.state == "empty" then
			emptys = emptys + 1
		elseif cell.state == "house" then
			houses = houses + 1
		elseif cell.state == "public" then
			publics = publics + 1
		end
	end)
	
	local total = #cs
	return (emptys * 100 / total), (houses * 100 / total), (publics * 100 / total)
end
	...
	init = function(model)
		...
		model.human = Agent{
			...
			init = function(self) 
				...
				if rand >= 100 - model.asymptomaticPercentage then
			...
			execute = function(self)
				if self.state == "infected" then
					-- if (self.incubationTime < self.infectivetyPeriod) or asymptomatic then
						-- local neigh = self:getCell():getNeighborhood():sample()
						-- if neigh.state ~= "empty" then
							-- self:move(neigh)
						-- end
					-- end 
				...
				-- else
					-- local rand = Random{min = 0, max = 100, step = 10}:sample()
		...
		model.vector = Agent{
			...
			incubationPeriod = 9,
			init = function(self)
				self.age = Random{min = 1, max = 40, step = 1}:sample()
			...
			execute = function(self)
				...
				elseif self.incubationTime > self.incubationPeriod then
					...
					if rand > 100 - model.choiceOfTargetPercentage then
						forEachElement(agts, function(idx, agt, _)
						...
						forEachNeighbor(cell, function(c, neigh)
							local neighAgents = neigh:getAgents()
							forEachElement(neighAgents, function(idx, agt, _)
								if (agt.name == "human") and (bits < model.bitsPerDay) then
									if agt.state == "susceptible" then
										agt.state = "infected"
										bits = bits + 1
		...
		local emptys, houses, publics = cellsOccupationPercetage(model.cs)
		
		print("Human Occupation Percentage: "..(houses + publics).."%")
		print("House Occupation Percentage: "..houses.."%")
		print("Public Occupation Percentage: "..publics.."%")
		print("Empty Percentage: "..emptys.."%")
		...
		forEachAgent(model.humans, function(human)
			...
			if (cell.state ~= "empty") and (numHumans(cell:getAgents()) < Nh) then
				human:enter(cell)
			else
				while (cell.state == "empty") or (numHumans(cell:getAgents()) >= Nh) do
		...
		forEachAgent(model.vectors, function(vector)
			local cell = model.cs:sample()
			if hasHuman(cell:getAgents()) and (numVectors(cell:getAgents()) < numHumans(cell:getAgents()) * model.maxDensityVectorPerHuman) then 
				vector:enter(cell)
			else
				while (not hasHuman(cell:getAgents())) or (numVectors(cell:getAgents()) >= numHumans(cell:getAgents()) * model.maxDensityVectorPerHuman) do
					cell = model.cs:sample()
				end
				vector:enter(cell)
			end		
		...
		model.timer = Timer{
			...
			Event{
				action = function(event)
					-- forEachCell(model.cs, function(c)
						local rand = Random{min = 0, max = 100, step = 1}:sample()
					...
					if event:getTime() == model.finalTime then
						print("Final Time: "..event:getTime(), "Number Of Humans: "..#model.humans, "Number Of Vectors: "..#model.vectors)
					end					
				end
			},
			Event{
